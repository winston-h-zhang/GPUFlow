...
template <typename U>
void print_result(const measurement_result<U> &res, std::string_view solver,
...
auto load_graph_and_run(std::istream &is, solver solver,
    ...
    case solver::prh: {
        auto [graph, source, sink, time_read] = get_graph_with_cached_edge(is);
        result = measure_sequential<push_relabel_highest::max_flow_instance>(
            std::move(graph), source, sink);
        result.time_read = time_read;
        thread_count = 1;
        break;
    ...
    case solver::prs: {
        auto [graph, source, sink, time_read] = get_graph_with_cached_edge(is);
        result = measure_parallel<push_relabel_segment::max_flow_instance>(
            std::move(graph), source, sink, thread_count);
        result.time_read = time_read;
        break;
    }
