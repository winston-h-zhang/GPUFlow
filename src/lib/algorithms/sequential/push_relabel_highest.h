...
class max_flow_instance {
    ...
    T _source, _sink, _highest_active, _highest_vertex, _relabel_progress,
        _relabel_threshold;

    // statistics
    uint64_t _push_cnt{0}, _relabel_cnt{0}, _gap_cnt{0}, _gap_nodes{0},
        _global_relabel_cnt{0};
  ...
  private:
        ...
        while (!_distance_q.empty()) {
            ...
            for (auto &edge : _residual_network[current_vertex]) {
                if (edge.reverse_r_capacity > 0 &&
                    ...
                    auto *node = &_vertices[edge.dst_vertex];
                    if (_vertices[edge.dst_vertex].excess > 0) {
                        _highest_active = std::max(
                            _highest_active, _vertices[edge.dst_vertex].label);
                        _labels[current_distance + 1].active_vertices.push(
                            node);
                    } else
                        _labels[current_distance + 1].inactive_vertices.push(
                            node);
                }
            }
        }
    ...
    void gap_relabel(const T gap_height) {
        ...
        for (auto current_height = gap_height + 1;
             current_height <= _highest_vertex; ++current_height) {
