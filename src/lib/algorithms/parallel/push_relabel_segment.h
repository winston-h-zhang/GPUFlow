...
/*
 * Parallel implementation of push-relabel algorithm, divides the network into
 * multiple segments.
 */
...
#pragma omp parallel for schedule(static)
        ...
        _original_relabel_threshold =
            (_residual_network.size() * ALPHA + m / 2);
    }
    ...
    void parallel_phase() {
        for (;;) {
            ...
            const auto partitions = partitioning::get_partitions(
                _labels, _highest_active, _thread_count, min_active_per_thread);
            ...
            if (actual_thread_cnt == 1) {
                push_relabel(thread_local_data{
                    cpu_time, 0, static_cast<T>(_residual_network.size()),
...
#pragma omp parallel for schedule(static) reduction(+:cpu_time) reduction(max:highest_active) reduction(max:highest_vertex)
            for (std::size_t i = 0; i < actual_thread_cnt; ++i) {
                ...
                push_relabel(thread_local_data{
                    cpu_time, low, high, highest_active, highest_vertex, 0});
                _relabel_threshold -=
                    _original_relabel_threshold / actual_thread_cnt;
                // add back vertices that are still active but couldn't have
                // been relabeled to higher partition
                _labels[high - 1].active_vertices.append_list(
                    _thread_local_labels[omp_get_thread_num()].active_vertices);
...
#pragma omp parallel for schedule(static)
    ...
    inline auto get_active_vertex(T &highest_active, const T low) noexcept {
        for (T i = 0; i < highest_active - low;
             ++i) // don't take vertices in low
        {
    ...
    inline void discharge(vertex *v, T label,
                          thread_local_data &data) noexcept {
    ...
    // original labels have to be set before the parallel phase starts and they
    // don't change until the next one
    inline bool same_thread(const T original_label, const T low,
                            const T high) const noexcept {
        return original_label >= low && original_label < high;
    }

    inline bool push(const T vertex, const T label,
                     const thread_local_data &data) noexcept {
        const auto target_label = label - 1;

        for (auto &edge : _residual_network[vertex]) {
            if (edge.r_capacity > 0 &&
                same_thread(_vertices[edge.dst_vertex].original_label, data.low,
                            data.high) &&
    ...
    inline T relabel(const T vertex, const T current_label,
        ...
        if (new_label < data.high) {
            ...
            // vertex is still active, but we cannot relabel it to another
            // partition, so we remember it and add it back to active vertices
            // at the end of this phase
            _thread_local_labels[omp_get_thread_num()].active_vertices.push(
                &_vertices[vertex]);
    ...
    inline T calculate_new_label(const T vertex,
        ...
        for (auto &edge : _residual_network[vertex]) {
            if (edge.r_capacity == 0 ||
                !same_thread(_vertices[edge.dst_vertex].original_label,
                             data.low, data.high))
                continue;
            increase_to =
...
#pragma omp parallel for schedule(static)
                    ...
                    if (edge.reverse_r_capacity > 0) {
                        if (!_vertices[edge.dst_vertex].discovered.test_and_set(
                            ...
                            _vertices[edge.dst_vertex].label =
                                current_distance + 1;
                            _vertices[edge.dst_vertex].original_label =
                                current_distance + 1;
                            ...
                            auto *node = &_vertices[edge.dst_vertex];
                            if (_vertices[edge.dst_vertex].excess > 0)
                                _thread_local_labels[thr_id]
                                    .active_vertices.push(node);
                            else
                                _thread_local_labels[thr_id]
                                    .inactive_vertices.push(node);
                        }
                    }
                }
            }
            ...
            for (std::size_t i = 0; i < _max_thread_count;
                 ++i) // append together all thread_local info
            {
                _labels[current_distance].active_vertices.append_list(
                    _thread_local_labels[i].active_vertices);
    ...
    void gap_relabel(const T gap_height,
                     const thread_local_data &data) noexcept {
        for (auto current_height = gap_height + 1;
             current_height <= data.highest_vertex; ++current_height) {
        ...
        data.highest_vertex = data.highest_active =
            std::max(gap_height - 1, data.low);
    }
};
} // namespace push_relabel_segment
