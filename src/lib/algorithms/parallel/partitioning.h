...
/*
 * Module for splitting network into multiple segments. Current implementation
 * divides the network into multiple segments of similar cost, where the cost of
 * a segment increases with it's height and the number of active vertices in it.
 */
...
namespace detail {
template <typename T> auto get_cost(T total_active, T height) noexcept {
    auto log = log2(height + 1);
    return height * log * log * total_active;
}

template <typename label_info, typename T>
auto get_next_partition(const std::unique_ptr<label_info[]> &labels,
                        const double cost_per_segment, const T low,
                        const T highest_active) noexcept {
    ...
    return std::make_tuple(highest_active + 1,
                           get_cost(total_active, highest_active - low));
}

template <typename label_info, typename T>
auto calculate_partitions(const std::unique_ptr<label_info[]> &labels,
                          const double cost_per_segment, const T highest_active,
    ...
    while (partitions.size() < thread_count) {
        auto [high, cost] =
            get_next_partition(labels, cost_per_segment, low, highest_active);
    ...
    double cost; // we have to use std::ignore, because otherwise we will get
                 // unused variable warning for the first member...
    std::tie(std::ignore, cost) = get_next_partition(
        labels, std::numeric_limits<double>::max(), low, highest_active);
    total_cost += cost;
    return total_cost;
}
} // namespace detail
...
template <typename label_info, typename T>
std::vector<T> get_partitions(const std::unique_ptr<label_info[]> &labels,
                              const T highest_active, std::size_t thread_count,
                              const T min_active_per_thread) noexcept {
    ...
    const double max_cost =
        detail::get_cost(total_active_cnt, highest_active + 1);
    ...
    while (low <= high) {
        ...
        auto total_cost = detail::calculate_partitions(
            labels, mid, highest_active, partitions, thread_count);
